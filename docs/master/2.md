...
该方法使用 X 的奇异值分解来计算最小二乘解。如果 X 是一个形状为 `(n_samples, n_features)`的矩阵，设![n_{samples} \geq n_{features}](img/1.1.1.1.(2).jpg), 则该方法的复杂度为![O(n_{samples} n_{fearures}^2)](img/1.1.1.1.(2).jpg)
